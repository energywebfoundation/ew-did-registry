...
describe('[DID-PROXY-OPERATOR]', function () {
  ...
  before(async () => {
    ...
    const proxyFactory = await (await proxyFactoryCreator.deploy(erc1056.address, { value: 1E15, })).deployed();
  ...
  //   const erc1056 = await (await erc1056Factory.deploy()).deployed();
  //   const proxyFactoryCreator = new ContractFactory(proxyFactoryAbi, proxyFactoryBytecode, deployer);
  //   const proxyFactory = await (await proxyFactoryCreator.deploy(erc1056.address, { gas: 10000000 })).deployed();
